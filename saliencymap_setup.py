"""
This file creates/reloads all the heatmap images at once
"""
...
def compute_saliency_map(model_name, displayed_class, number_image):
    ...
    image_path = "models/test_" + displayed_class + "_images/" + str(number_image)
    ...
    save_destination = 'images/' + model_name + '/' + str(number_image[:-4]) + '_saliency.png'
    
    plt.savefig(save_destination)
    plt.clf()

# Get all the pretrained model names
pretrained_models = []
model_files = os.listdir('./models/model_saves')
for model in model_files:
    if(model[-4:] == ".pth"):
        pretrained_models.append(model[:-4])
        print("Model saved: ", model)
print(pretrained_models)

# Get all the displayed classes (dog and cat)
displayed_classes = ["dog", "cat"]

# Get all the numbers of images
images_dog = os.listdir('./models/test_dog_images')
images_cat = os.listdir('./models/test_cat_images')
print(images_cat)

# Launch all the main functions
for model in pretrained_models:
    for displayed_class in displayed_classes:
        if(displayed_class == "dog"):
            for image in images_dog:
                compute_saliency_map(model, displayed_class, image)
        elif(displayed_class == "cat"):
